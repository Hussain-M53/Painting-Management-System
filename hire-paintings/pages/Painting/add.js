...
function add() {
  ...
  const push_data_to_db = async () => {
    axios
      .post("/Painting", {
        p_title: title.current.value,
        p_theme: theme.current.value,
        p_rental_price: rental_price.current.value,
        p_artist_id: artist_id.current.value,
        p_owner_id: owner_id.current.value,
      })
      .then((res) => {
        alert(res.data);
      })
